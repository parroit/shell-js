...
function ShellController(shellView) {
	...
	_this.view.events.on("command", function(command) {
		_this.findCommandRunner(command, function(runner) {
			_this.currentRunner = runner;
			runner.events.on("inputRequested",function(){
			...
			function sendDataToView(chunk) {
				
				//console.log("stdout.on data")
				var out = chunk.toString("utf8");
				//console.log(out);
				ansi2html.parseChunk(out,function(html){
					//console.log(html)
			...
			io.stdout.on("data", sendDataToView);
			io.stderr.on("data", sendDataToView);

			io.stdout.on("end", function() {
				//console.log("stdout.on end")
				_this.view.write("<br></span>");
...
methods.sendInput = function(keycode){
	this.currentRunner.sendInput(keycode);
}
